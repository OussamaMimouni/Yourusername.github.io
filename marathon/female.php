...
try {
	...
	$sql="SELECT * FROM athlete  WHERE sexAthlete = 'female'";
...
<table class="table" align="center" style="width: 900PX;height: 480px;top: 100px;background-color: rgb(290, 290, 290 ,0.66);text-align: center;">
  <thead class="table-dark">
  	<th>nom</th><th>prenom</th><th>age</th><th>pays</th><th>meilleur chrono</th>
  </thead>
  <tbody>
<?php
while($tabAthlete=$result->Fetch()) {
?>
<tr><td><?=$tabAthlete['nomAthlete']?></td><td><?=$tabAthlete['prenomAthlete']?></td><td><?=$tabAthlete['ageAthlete']?></td><td><?=$tabAthlete['payAthlete']?></td><td><?=$tabAthlete['mcAthlete']?></td></tr>
<?php
}
?>
  </tbody>
